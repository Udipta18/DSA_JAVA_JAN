...
public class word_search {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<maze.length;i++) {
			for(int j=0;j<maze[0].length;j++) {
				...
				boolean ans=	print(maze,i,j,word,0);
				if(ans==true) {
	...
	public static boolean print(char[][] maze,int cr,int cc,String word,int idx) {
		...
		for(int i=0;i<c.length;i++) {
			boolean ans=print(maze,cc+c[i],cr+r[i],word,idx+1);
			if(ans==true) {
				return true;
			}
		}
